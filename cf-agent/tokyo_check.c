...
/*
 * The idea behind this code comes from : copiousfreetime@github
 */
...
static db_meta_t* dbmeta_new( const char* dbfilename )
{
  TCHDB     *hdb;
  db_meta_t *dbmeta;
  int       errnum;
  ...
  if(!dbmeta) {CfOut(OUTPUT_LEVEL_ERROR, "", "Error allocating memory\n");}
  ...
  if ( !tchdbopen( hdb, dbmeta->dbpath , HDBOREADER )) {
    ...
    CfOut(OUTPUT_LEVEL_ERROR, "", "Failure opening database [%s] : %s\n", dbmeta->dbpath, tchdberrmsg( errnum ));
    tchdbdel( hdb );
    exit( 1 );
  }
...
static void add_offset_to_tree_unless_exists( rbtree** tree, uint64_t offset, int64_t bucket_index )
{
  rbtree *other = NULL;

  rbtree *new_node       = (rbtree*)xcalloc( 1, sizeof( rbtree ));
  if(!new_node) {CfOut(OUTPUT_LEVEL_INFORM, "", "BIG PROBLEM\n");}
  ...
  if ( NULL == other ) {
    ...
    if(!new_node) {CfOut(OUTPUT_LEVEL_INFORM, "", "BIG PROBLEM2\n");}
    free( new_node );
  }

}

static void dbmeta_populate_offset_tree( db_meta_t* dbmeta )
{
  uint64_t i;

  lseek( dbmeta->fd, dbmeta->bucket_offset, SEEK_SET );
  //  CfOut(OUTPUT_LEVEL_INFORM, "", "Error seeking : %s\n", strerror(errno));
  
  CfOut(OUTPUT_LEVEL_ERROR, "", "Traversing bucket section to find record offsets : \n" );
 ...
 CfOut(OUTPUT_LEVEL_ERROR, "", "Found %llu buckets with offsets\n", (long long unsigned)sglib_rbtree_len( dbmeta->offset_tree ));
 return;
}
...
{
  lseek( dbmeta->fd, rec->offset, SEEK_SET );
  ...
  while( true ) {
    ...
    if ( MAGIC_DATA_BLOCK ==  rec->magic ) {
      CfOut(OUTPUT_LEVEL_INFORM, "", "off=%llu[c8]\n", rec->offset);
      int length = 1;
      ...
      //CfOut(OUTPUT_LEVEL_INFORM, "", "off=%llu[b0]\n", rec->offset);
...
static bool dbmeta_populate_record_tree( db_meta_t* dbmeta )
{
  off_t   offset;
  ...
  while( offset < st.st_size ) {
    ...
    if( !dbmeta_read_one_rec( dbmeta, &new_rec )) { 
      CfOut(OUTPUT_LEVEL_ERROR, "", "Unable to find a record at the file is finished\n");
      return false;
    } else {
    ...
    if ( MAGIC_DATA_BLOCK == new_rec.magic ) {
      ...
      if ( new_rec.left > 0 ) {
        //CfOut(OUTPUT_LEVEL_INFORM, "", ">>> handle left %x\n", new_rec.left);
        add_offset_to_tree_unless_exists( &(dbmeta->offset_tree), new_rec.left, -1 );
      }

      if ( new_rec.right > 0 ) {
        //CfOut(OUTPUT_LEVEL_INFORM, "", ">>> handle right %x\n", new_rec.right);
  ...
  CfOut(OUTPUT_LEVEL_INFORM, "",  "Found %llu data records and %llu free block records\n", data_blocks, free_blocks);
...
static int dbmeta_get_results( db_meta_t *dbmeta )
{
  ...
  CfOut(OUTPUT_LEVEL_INFORM, "", "Found %llu offsets listed in buckets that do not have records\n", buckets_no_record);
  CfOut(OUTPUT_LEVEL_INFORM, "", "Found %llu records in data that do not have an offset pointing to them\n", records_no_bucket);
...
int CheckTokyoDBCoherence( char *path )
{
  ...
  CfOut(OUTPUT_LEVEL_INFORM, "", "Populating with bucket section offsets\n");
  dbmeta_populate_offset_tree( dbmeta );
  CfOut(OUTPUT_LEVEL_INFORM, "", "Populating with record section offsets\n");

...
DBPriv *DBPrivOpenDB(const char *dbpath)
{
    ...
    if (rc)
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! Could not create handle for database %s: %s",
    ...
    if (rc)
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! Could not set mapsize for database %s: %s",
              dbpath, mdb_strerror(rc));
        goto err;
    }
    ...
    if (rc)
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! Could not open database txn %s: %s",
              dbpath, mdb_strerror(rc));
        goto err;
    }
    ...
    if (rc)
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! Could not open database dbi %s: %s",
              dbpath, mdb_strerror(rc));
        goto err;
    }
    ...
    if (rc)
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! Could not commit database dbi %s: %s",
              dbpath, mdb_strerror(rc));
        goto err;
    }
    txn = NULL;
...
bool DBPrivHasKey(DBPriv *db, const void *key, int key_size)
{
    ...
    if (rc == MDB_SUCCESS)
    {
        ...
        {
            CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not read: %s", mdb_strerror(rc));
        }
        mdb_txn_abort(txn);
    }
    else
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not create read txn: %s", mdb_strerror(rc));
    }
...
bool DBPrivWrite(DBPriv *db, const void *key, int key_size, const void *value, int value_size)
{
    ...
    if (rc == MDB_SUCCESS)
    {
        ...
        if (!db->mc)
        {
            if (rc == MDB_SUCCESS)
            {
                ...
                if (rc)
                {
                    CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not commit: %s", mdb_strerror(rc));
                }
            }
            else
            {
                CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not write: %s", mdb_strerror(rc));
                mdb_txn_abort(txn);
            }
        }
    }
    ...
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not create write txn: %s", mdb_strerror(rc));
    }
    return rc == MDB_SUCCESS;
}
...
bool DBPrivDelete(DBPriv *db, const void *key, int key_size)
{
    ...
    if (rc == MDB_SUCCESS)
    {
        ...
        if (!db->mc)
        {
            ...
            {
                CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not delete: %s", mdb_strerror(rc));
                mdb_txn_abort(txn);
            }
        }
    }
...
DBCursorPriv *DBPrivOpenCursor(DBPriv *db)
{
    ...
    if (rc == MDB_SUCCESS)
    {
        ...
        {
            CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not open cursor: %s", mdb_strerror(rc));
            mdb_txn_abort(txn);
        }
    ...
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not create cursor txn: %s", mdb_strerror(rc));
    }
                     ...
                     void **value, int *value_size)
{
    ...
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not advance cursor: %s", mdb_strerror(rc));
    }
...
bool DBPrivWriteCursorEntry(DBCursorPriv *cursor, const void *value, int value_size)
{
    ...
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not write cursor entry: %s", mdb_strerror(rc));
    }
    return rc == MDB_SUCCESS;
}
...
void DBPrivCloseCursor(DBCursorPriv *cursor)
{
    ...
    if (rc)
    {
        CfOut(OUTPUT_LEVEL_ERROR, "", "!! could not commit cursor txn: %s", mdb_strerror(rc));
    }
    free(cursor);
}

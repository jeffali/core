...
{
  //printf("lololololo %x %lld %lld\n", tree, offset, bucket_index);
  rbtree *other = NULL;

  rbtree *new_node       = (rbtree*)calloc( 1, sizeof( rbtree ));
  if(!new_node) {printf("BIG PROBLEM\n");}
  ...
  if ( NULL == other ) {
    ...
    if(!new_node) {printf("BIG PROBLEM2\n");}
    free( new_node );
  }
